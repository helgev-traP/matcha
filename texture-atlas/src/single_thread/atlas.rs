...
impl Drop for TextureInner {
    fn drop(&mut self) {
        if let Some(atlas) = self.atlas.upgrade() {
            match atlas.borrow_mut().deallocate(self.texture_id) {
                Ok(_) => {
...
struct TextureAtlasState {
    allocators: Vec<RefCell<AtlasAllocator>>,
...
impl TextureAtlas {
    ...
    pub fn max_allocation_size(&self) -> [u32; 2] {
        ...
        for entry in self.state.texture_id_to_location.values() {
            let size = entry.size();
...
impl TextureAtlas {
    pub fn allocate(&mut self, size: [u32; 2]) -> Result<Texture, TextureAtlasError> {
        let size_req = Size::new(size[0] as i32, size[1] as i32);

        for (page_index, allocator) in self.state.allocators.iter().enumerate() {
            if let Some(alloc) = allocator.borrow_mut().allocate(size_req) {
...
impl TextureAtlas {
    pub fn resize(
        ...
        let (mut new_texture_id_to_location, mut new_texture_id_to_alloc_id, mut new_usage) =
        ...
        let location_map = new_texture_id_to_location.iter().map(|(id, new_location)| {
    ...
    fn reallocate_existing(
        &self,
        new_allocators: &mut [RefCell<AtlasAllocator>],
        new_atlas_size: wgpu::Extent3d,
        allow_data_loss: bool,
    ) -> Result<
        (
            HashMap<TextureId, TextureLocation>,
            HashMap<TextureId, AllocId>,
            usize,
        ),
        TextureAtlasError,
    > {
        let mut new_texture_id_to_location = HashMap::new();
        let mut new_texture_id_to_alloc_id = HashMap::new();
        let mut new_usage = 0;
...
mod tests {
    ...
    fn test_texture_allocation_and_deallocation() {
        pollster::block_on(async {
            ...
            let texture1 = atlas.borrow_mut().allocate([32, 32]).unwrap();
            assert_eq!(atlas.borrow().allocation_count(), 1);
            assert_eq!(atlas.borrow().usage(), 32 * 32);

            let texture2 = atlas.borrow_mut().allocate([16, 16]).unwrap();
            assert_eq!(atlas.borrow().allocation_count(), 2);
            assert_eq!(atlas.borrow().usage(), 32 * 32 + 16 * 16);

            drop(texture1);
            assert_eq!(atlas.borrow().allocation_count(), 1);
            assert_eq!(atlas.borrow().usage(), 16 * 16);

            drop(texture2);
            assert_eq!(atlas.borrow().allocation_count(), 0);
            assert_eq!(atlas.borrow().usage(), 0);
        });
    }
    ...
    fn test_allocation_failure() {
        pollster::block_on(async {
            ...
            let _texture1 = atlas.borrow_mut().allocate([32, 32]).unwrap();

            let result = atlas.borrow_mut().allocate([1, 1]);
            assert!(matches!(
                result,
                Err(TextureAtlasError::AllocationFailedNotEnoughSpace)
            ));
    ...
    fn test_reuse_deallocated_space() {
        pollster::block_on(async {
            ...
            let texture1 = atlas.borrow_mut().allocate([64, 64]).unwrap();
            assert_eq!(atlas.borrow().allocation_count(), 1);

            drop(texture1);
            assert_eq!(atlas.borrow().allocation_count(), 0);

            let _texture2 = atlas.borrow_mut().allocate([64, 64]).unwrap();
            assert_eq!(atlas.borrow().allocation_count(), 1);
        });
    }
    ...
    fn test_atlas_resize_grow() {
        pollster::block_on(async {
            ...
            assert_eq!(atlas.borrow().size(), new_size);
            assert_eq!(atlas.borrow().allocation_count(), 1);
        });
    }
    ...
    fn test_atlas_resize_shrink_fail() {
        pollster::block_on(async {
            ...
            let _texture1 = atlas.borrow_mut().allocate([64, 64]).unwrap();
            ...
            assert_eq!(atlas.borrow().size(), initial_size);
            assert_eq!(atlas.borrow().allocation_count(), 1);
        });
    }
    ...
    fn test_atlas_resize_shrink_with_data_loss() {
        pollster::block_on(async {
            ...
            let _texture1 = atlas.borrow_mut().allocate([40, 40]).unwrap();
    ...
    fn test_texture_uv() {
        pollster::block_on(async {
            ...
            let texture = atlas.borrow_mut().allocate([32, 64]).unwrap();
    ...
    fn test_texture_error_when_atlas_gone() {
        pollster::block_on(async {
            ...
            let texture = atlas.borrow_mut().allocate([32, 32]).unwrap();
    ...
    fn test_texture_write_and_read_data() {
        pollster::block_on(async {
            ...
            let _texture1 = atlas.borrow_mut().allocate([10, 10]).unwrap();
            let texture2 = atlas.borrow_mut().allocate([17, 17]).unwrap();
            ...
            encoder.copy_texture_to_buffer(
                wgpu::TexelCopyTextureInfo {
                    texture: &atlas.borrow().textures()[0],
                    mip_level: 0,
