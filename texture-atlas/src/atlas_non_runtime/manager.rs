...
use crate::atlas_non_runtime::{Texture, TextureAtlas, TextureAtlasError};
...
pub struct AtlasManager {
    ...
    atlases: DashMap<Vec<wgpu::TextureFormat>, Arc<RwLock<TextureAtlas>>>,
}
...
impl AtlasManager {
    ...
    pub fn allocate(
        ...
        let atlas = atlas.upgradable_read();

        // check if the atlas need to be resized proactively
        ...
        if !need_resize {
            // No resize needed, try to allocate with a read lock.
            if let Ok(texture) = atlas.allocate(size) {
                    ...
                    == self.max_size_of_3d_texture.depth_or_array_layers
                {
                    return Err(AtlasManagerError::from(
                        crate::atlas_non_runtime::TextureAtlasError::AllocationFailedNotEnoughSpace,
                    ));
                }
            }
        }

        let mut atlas = RwLockUpgradableReadGuard::upgrade(atlas);

        // Check again after upgrading the lock, in case another thread resized it.
        // check resize threshold again
        let mut resize = None;
        if let Some(resize_threshold) = self.memory_strategy.resize_threshold {
            let atlas_size = atlas.size();
            let capacity = atlas.capacity();
        ...
        if resize.is_none() {
            // No resize needed, try to allocate with a write lock.
            if let Ok(texture) = atlas.allocate(size) {
                return Ok(texture);
            } else {
                let atlas_size = atlas.size();
        ...
        match atlas.resize(&self.device, &self.queue, new_size, false, Some(size)) {
            ...
            Err(e) => {
                // The resize itself failed. We can propagate the error.
                Err(e.into())
            }
        }
    }
...
mod tests {
    ...
    impl AtlasManager {
        ...
        fn get_atlas_size(&self, formats: &[wgpu::TextureFormat]) -> Option<wgpu::Extent3d> {
            self.atlases.get(formats).map(|atlas| atlas.read().size())
        }

        fn get_atlas_usage(&self, formats: &[wgpu::TextureFormat]) -> Option<usize> {
            self.atlases.get(formats).map(|atlas| atlas.read().usage())
        }
    }
    ...
    /// Tests automatic resizing (growing) of the atlas.
    #[test]
    fn test_proactive_resize_only() {
        pollster::block_on(async {
            ...
            // Allocate a texture that fills the 1/4 of the atlas.
            // This should trigger a resize.
            let _texture = manager.allocate([32, 32], &formats).unwrap();

            // After this allocation, the atlas should have resized to 2 pages.
            assert_eq!(
                manager
                    .get_atlas_size(&formats)
                    .unwrap()
    ...
    /// Tests that allocation fails when the atlas reaches its maximum allowed size.
    #[test]
    fn test_allocate_max_size_reached() {
        pollster::block_on(async {
            ...
            let memory_strategy = MemoryAllocateStrategy {
                initial_pages: 1,
                resize_threshold: Some(0.1), // Low threshold to force quick resize
                resize_factor: 2.0,
                shrink_threshold: 0.2,
                shrink_factor: 0.5,
            };
            let max_size = wgpu::Extent3d {
                width: 32,
                height: 32,
                depth_or_array_layers: 1, // Max 1 page
            };
            let manager =
                AtlasManager::new(Arc::new(device), Arc::new(queue), memory_strategy, max_size);
            ...
            // Allocate a texture that fills the single page
            ...
            // Attempt to allocate another texture, should fail as max size is reached
